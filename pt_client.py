...
def get_data(split="all"):
    ...
    if split == "all":
        ...
        print("error: split can be 'all', 'alice', 'bob', or 'val'. Default 'all'")
        return None
...
class PTMLPClient(fl.client.NumPyClient, nn.Module):
    ...
    def __init__(self, dim_in=4, dim_h=32, num_classes=3, lr=1e-4, split="alice"):
    ...
    def fit(self, parameters, config=None, epochs=10):
        ...
        return self.get_parameters(), len(y), \
                {"loss": loss, "accuracy": accuracy_dict["accuracy"]}
    ...
    def evaluate(self, parameters, config=None):
        ...
        val_x, val_y = torch.tensor(val_x).float(), torch.tensor(val_y).long()
        
        self.eval()
        prediction = self.forward(val_x)
        loss = self.loss_fn(prediction, val_y).detach().numpy()
        
        prediction_class = np.argmax(prediction.detach().numpy(), axis=-1)
        
        accuracy = sklearn.metrics.accuracy_score(val_y.numpy(), prediction_class)
